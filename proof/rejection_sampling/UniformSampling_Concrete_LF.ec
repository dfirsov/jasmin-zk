...
lemma rsample_leakages2 a  &m : (glob M1){m} = [] =>
  Pr[ M1.bn_rsample(a) @ &m : M1.leakages <> samp_t res.`1 ] = 0%r.
...
lemma samp_t_correct a l x &m : (glob M1){m} = [] =>
  Pr[ M1.bn_rsample(a) @ &m : M1.leakages = l  /\ res.`2 = x ]
  = Pr[ M1.bn_rsample(a) @ &m : l = samp_t res.`1 /\ res.`2 = x  ].
move => H.
have ->: Pr[ M1.bn_rsample(a) @ &m : l = samp_t res.`1 /\ res.`2 = x ]
 = Pr[ M1.bn_rsample(a) @ &m : l = samp_t res.`1 /\ res.`2 = x /\ M1.leakages = samp_t res.`1  ].
  have -> : Pr[M1.bn_rsample(a) @ &m : l = samp_t res.`1 /\ res.`2 = x]
   = Pr[M1.bn_rsample(a) @ &m : l = samp_t res.`1 /\ res.`2 = x /\ M1.leakages = samp_t res.`1]
   + Pr[M1.bn_rsample(a) @ &m : l = samp_t res.`1 /\ res.`2 = x /\ M1.leakages <> samp_t res.`1].
  ...
  have ->: Pr[M1.bn_rsample(a) @ &m :
   l = samp_t res.`1 /\ res.`2 = x /\ M1.leakages <> samp_t res.`1] = 0%r.
    have : Pr[M1.bn_rsample(a) @ &m :
     ...
     <= Pr[ M1.bn_rsample(a) @ &m : M1.leakages <> samp_t res.`1 ]. smt(@Distr).
    rewrite (rsample_leakages2 a &m). auto. smt(@Distr). 
auto.
have -> : Pr[ M1.bn_rsample(a) @ &m : l = samp_t res.`1 /\ res.`2 = x /\ M1.leakages = samp_t res.`1  ]
 = Pr[ M1.bn_rsample(a) @ &m : res.`2 = x /\  l = M1.leakages /\ M1.leakages = samp_t res.`1  ].
rewrite Pr[mu_eq]. move => &hr. split.  smt().  smt(). auto.
have -> : Pr[ M1.bn_rsample(a) @ &m : res.`2 = x /\ l = M1.leakages /\ M1.leakages = samp_t res.`1  ]
 = Pr[ M1.bn_rsample(a) @ &m :  l = M1.leakages /\ res.`2 = x ].
  have : Pr[ M1.bn_rsample(a) @ &m :  l = M1.leakages /\ res.`2 = x ] 
   = Pr[ M1.bn_rsample(a) @ &m :  l = M1.leakages /\ res.`2 = x /\ M1.leakages = samp_t res.`1  ]
   + Pr[ M1.bn_rsample(a) @ &m :  l = M1.leakages /\ res.`2 = x /\ M1.leakages <> samp_t res.`1  ].
 rewrite Pr[mu_split (M1.leakages = samp_t res.`1 )]. smt().
   have : Pr[ M1.bn_rsample(a) @ &m :  l = M1.leakages /\ res.`2 = x /\ M1.leakages <> samp_t res.`1  ]
    = 0%r.
     have : Pr[ M1.bn_rsample(a) @ &m :  l = M1.leakages /\ res.`2 = x /\ M1.leakages <> samp_t res.`1  ] 
         <= Pr[ M1.bn_rsample(a) @ &m : M1.leakages <> samp_t res.`1 ].
...
(* rsample_leakf *)
lemma bn_rsample_leakf a x l &m : (glob M1){m} = [] 
 => Pr[ M1.bn_rsample(a) @ &m : M1.leakages = l /\ res.`2 = x  ]
...
lemma bn_rsample_v &m x a l: (glob M1){m} = [] 
  => Pr[ M1.bn_rsample(a) @ &m : M1.leakages = l  /\ res.`2 = x  ] 
       = if (valR a <= valR x) then 0%r else (g a l).
      progress.
   rewrite  (bn_rsample_leakf a x l &m);auto.
...
lemma bn_rsample_leakfree pin l a &m: (glob M1){m} = [] 
 =>  let v = Pr[ M1.bn_rsample(pin) @ &m : res.`2 = a  ] in 
     let w = Pr[ M1.bn_rsample(pin) @ &m : M1.leakages = l  /\ res.`2 = a  ] in 
  0%r < w => v/w 
  = rsample_f pin l.
move => l_empty v w f1.
