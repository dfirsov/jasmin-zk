...
declare module P <: Prog.
op p_fun : int -> int * glob P.

declare axiom p_loss : forall &m x, Pr[P.main(x) @ &m : true] = 1%r.

declare axiom p_deterministic &m x :
  Pr[ P.main(x)@&m : (res, glob P) = p_fun x  ] = 1%r.


declare axiom p_ct &m x1 x2 g:
  Pr[ P.main(x1)@&m : (glob P) = g ]
 = Pr[ P.main(x2)@&m : (glob P) = g ].

lemma g_all_same x1 x2 : snd (p_fun x1) = snd (p_fun x2).
proof. 
...
op g_fun' (g : glob P) : int option = choiceb 
 (fun i =>  i <> None /\ snd (p_fun (oget i)) = g) 
  None.

op g_fun (g : glob P) : real = 
 if g_fun' g = None then 0%r else 1%r.

lemma p_ct_gen &m x r g: 
...
progress.
  ...
  rewrite Pr[mu_not]. rewrite p_loss.
...
have claim2 : (r,g) = p_fun x. 
  ...
  move => hh.
    ...
    rewrite - (p_deterministic &m x). smt.
...
have : g_fun' g <> None. 
rewrite /g_fun'.
pose P := (fun (i : int option) => i <> None /\ (p_fun (oget i)).`2 = g).
have cl : exists x0, P x0.
exists (Some x). rewrite /P. split. smt().
simplify. apply h. smt.

move => hh.
have : choiceb (fun (i : int option) => i <> None /\ (p_fun (oget i)).`2 = g)
     None <>
   None. smt.
have -> : (if choiceb (fun (i : int option) => i <> None /\ (p_fun (oget i)).`2 = g)
     None =
   None then 0%r
else 1%r) = 1%r. smt().
move => _.
...
rewrite p_deterministic. auto.
 have q2 : Pr[P.main(x) @ &m : (res, glob P) <> (p_fun x)]  = 0%r. rewrite Pr[mu_not]. rewrite p_loss. rewrite q. auto.
