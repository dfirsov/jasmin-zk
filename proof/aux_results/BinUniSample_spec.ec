...
(* rewrite duniformE. *)
(* have ->: (undup (range 0 (7 + 1)))  *)
(*  = (range 0 (7 + 1)). smt(@List). *)
(* have ->: (size (range 0 (7 + 1))) = 8. smt(@List). *)
(* have ->: (count (fun (x : int) => x %% 2 = 0) (range 0 (7 + 1))) = 4.  *)
(* rewrite range_ltn; first smt();  simplify; simplify. *)
(* rewrite range_ltn; first smt();  simplify. simplify. *)
(* rewrite range_ltn; first smt();  simplify. simplify. *)
(* rewrite range_ltn; first smt();  simplify. simplify. *)
(* rewrite range_ltn; first smt();  simplify. simplify. *)
...
(* rewrite range_geq. auto. auto. auto.  *)
(* qed. *)
