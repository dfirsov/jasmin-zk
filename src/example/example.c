#include <string.h>
#include <stdint.h>
#include <inttypes.h>
#include <stdio.h>
#include <assert.h>

#include "../schnorr_protocol.h"

uint8_t statement[256] = {
  
};

uint8_t witness[256] = {
  
};


int main(void)
{
  /* uint64_t commitment_p[NLIMBS],  */
  /*   random_power_p[NLIMBS],  */
  /*   challenge_p[NLIMBS], */
  /*   response_p[NLIMBS], */
  /*   witness_p[NLIMBS], */
  /*   statement_p[NLIMBS], */
  /*   result_p[NLIMBS]; */

  bignum commitment_p, random_power_p, challenge_p, response_p, witness_p, statement_p;
  uint64_t result_p[1];

  //  bignum commitment_p = commitment, random_power_p, challenge_p, response_p, witness_p = witness, statement_p, result_p;

  
  printf("size: %ld\n", sizeof(bignum)); // TEMPORARY, to check
  
  memcpy(statement_p, statement, sizeof(bignum));
  memcpy(witness_p, witness, sizeof(bignum));
  //  load(statement_p, statement);
  //  load(witness_p, witness);

  printf("statement copy check: %d\n", !memcmp(statement_p, statement, sizeof(bignum)));
  printf("witness copy check: %d\n", !memcmp(witness_p, witness, sizeof(bignum)));

  // OR: directly pass commitment, witness to the functions below
  
   __commitment(commitment_p,random_power_p);
  /* printf("1\n"); */
   __challenge(challenge_p);
  /* printf("2\n"); */
   __response(random_power_p,challenge_p,witness_p,response_p);
  /* printf("3\n"); */
  __verify(statement_p,commitment_p,challenge_p,response_p,result_p);
  /* printf("4\n"); */

  printf("0x%ld\n", result_p[0]);
  return 0;
}
