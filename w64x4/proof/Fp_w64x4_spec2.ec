...
lemma pVal: P = 13. by smt(pE two_pow255E). qed.
...
lemma twop255_cgr : 2^255 %% P = 8 by smt.

lemma zpcgr_over a b:
 zpcgr (a + 2^255 * b) (a + 8 * b).
proof.
smt.
qed.

lemma inzp_over x:
 inzp (2^255 * x) = inzp (8*x).
proof. by have /= := zpcgr_over 0 x; rewrite -eq_inzp. qed.

lemma twop256_cgr : 2^256 %% P = 3 by smt.

lemma P_cgr: (2^256 - 2^255 - 8)%%P = 0.
proof.
rewrite -modzDml -(modzDm (2^256) (-2^255)) -modzNm
     twop256_cgr twop255_cgr modzDmr /= modzDml. smt.
qed.

(* lemma ltP_overflow x: *)
(*  (x + 2^255 + 8 < 2^256) = (x < P). *)
(* proof.  *)
(* have ->: 2^255 = P + 8. rewrite pE.  smt(twop255_cgr). *)
(* have ->: 2^256 = P + P + 8 + 8 by rewrite !pE /#. *)
(* smt(). *)
(* qed. *)

op red x = if x + 2^255 + 8 < 2^256 then x else (x + 2^255 + 8) %% 2^256.
...
pose y := x-P.
 ...
 rewrite -addrA -addrA (_:P + (P + 8 + 8) = 2^256) 1:pE 1:/#. *)
rewrite  modz_small.
apply bound_abs.
move: H2; have ->: 2^256 = P + P + 8 + 8 by rewrite !pE .
smt(pVal).
qed.
